...
- 围绕 OpenAI-compatible Profile 重构 API 配置。文本模型预设限制为 `gpt-5.4` 和 `gpt-5.5`，图片模型预设为 `gpt-image-2`。
- 改进移动端适配，覆盖主工作区、顶部控制区、API 入口和 API 设置面板。
- 移除旧的代理推荐内容，以及与当前 OpenAI-compatible API 流程不匹配的旧兼容路径。
- 修复 DEV 请求监控清空功能，并确保再生图片历史和详情中保留最新的再生 prompt。
- 增加生成辅助函数、项目配置快照和再生元数据的回归测试。
